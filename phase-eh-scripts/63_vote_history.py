...
"""
Task #63: 予想履歴ページ
- src/app/(main)/mypage/votes/page.tsx: 全投票履歴一覧
- マイページメニューにリンク追加
"""
...
def run():
    # 1. ページ作成
    os.makedirs("src/app/(main)/mypage/votes", exist_ok=True)
    with open("src/app/(main)/mypage/votes/page.tsx", "w") as f:
        f.write(VOTES_PAGE)
    print("  ✅ src/app/(main)/mypage/votes/page.tsx")
    ...
    if os.path.exists(mypage):
        ...
        if "/mypage/votes" not in content:
            old = '<MenuItem href="/mypage/points" icon="💰" label="ポイント履歴" desc="獲得ポイントの詳細" />'
