...
"""
Task #40: 馬カルテページ
- /horses/[horseId]/page.tsx: 過去成績・戦績一覧
- レース詳細の馬名からリンク
"""
...
horse_dir = "src/app/(main)/horses/[horseId]"
os.makedirs(horse_dir, exist_ok=True)
with open(f"{horse_dir}/page.tsx", "w") as f:
    f.write(HORSE_PAGE)
print(f"✅ {horse_dir}/page.tsx")

# ============================================================
# 2. HorseList に馬カルテリンクを追加
# ============================================================
horse_list = "src/components/races/HorseList.tsx"
if os.path.exists(horse_list):
    with open(horse_list, "r") as f:
        content = f.read()

    if "/horses/" not in content:
        # 馬名の部分を Link で囲む
        # パターン: {entry.horses?.name} を探す
        old_pattern = "entry.horses?.name"
        if old_pattern in content:
            # Link import追加
            if 'import Link' not in content:
                content = 'import Link from "next/link";\n' + content
            # 馬名をリンク化（完全なパターンが必要なので注意書きだけ）
            print(f"⚠️  {horse_list}: 馬名にリンクを手動で追加してください")
            print("    例: <Link href={`/horses/${entry.horses?.id}`} className=\"text-green-600 hover:underline\">{entry.horses?.name}</Link>")
        else:
            print(f"⚠️  {horse_list}: パターン不一致")
    else:
        print(f"⏭️  {horse_list}: 既にリンクあり")
else:
    print(f"⚠️  {horse_list} が見つかりません")

# ============================================================
# 3. VoteForm の馬名にもリンク追加（情報として表示）
# ============================================================
# VoteFormはクライアントコンポーネントなので、投票中にページ遷移はUX的に微妙
# 代わりに出馬表側でリンクを追加済みなので、ここではスキップ

print("\n🏁 Task #40 完了")
print("📌 馬名リンクは HorseList.tsx と出馬表で手動追加を推奨")
