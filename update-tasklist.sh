...
## ✅ Phase I-polish: テストリリース前調整（完了）

> Phase I完了後のフィードバック対応・細かい調整（2026-02-15 完了）

| # | タスク | 説明 | 状態 | 完了日 |
|---|--------|------|------|--------|
| 77 | TL投票表示改善 | 投票時に◎○△の馬名をTLに表示。未確定/確定両対応 | ✅済 | 2026-02-15 |
| 78 | 通知システム整理 | follows/reactions/repliesのみに絞り、createNotification共通関数化 | ✅済 | 2026-02-15 |
| 79 | 大会ページ「近日開催予定」化 | Cron停止 + 大会概要ティザー表示 + TOPバナー差し替え | ✅済 | 2026-02-15 |
| 80 | 管理KPIダッシュボード | DAU/投票数/コメント数/通報数/7日間推移グラフ | ✅済 | 2026-02-15 |

---

## ✅ Phase K: データ自動化・ポイント改定・UX改善（完了）

> レース運用の自動化とゲームバランス調整（2026-02-15 完了）

| # | タスク | 説明 | 状態 | 完了日 |
|---|--------|------|------|--------|
| 81 | レース結果自動取得 | netkeibaから結果スクレイプ + 管理画面ワンクリック承認UI | ✅済 | 2026-02-15 |
| 82 | レース結果自動清算Cron | 発走10分後に自動で結果取得→清算。最大3回リトライ（10分おき） | ✅済 | 2026-02-15 |
| 83 | レース情報自動更新Cron | 土日9-17時JST 30分おき。オッズ・除外馬・騎手変更を自動更新 | ✅済 | 2026-02-15 |
| 84 | ポイントシステム改定 | グレード別ボーナス（G1+30/G2+15/G3+10/OP+5）、危険馬人気傾斜（1番人気50P〜） | ✅済 | 2026-02-15 |
| 85 | ランク閾値調整 | レジェンド100,000P→45,000P。到達目安9〜12ヶ月に設計 | ✅済 | 2026-02-15 |
| 86 | ポイントガイドページ更新 | 新ルール反映（グレード別/危険馬人気別テーブル、計算例追加） | ✅済 | 2026-02-15 |
| 87 | 戻るナビゲーション追加 | BackLink共通コンポーネント。下層ページ16箇所に「← 戻る」追加 | ✅済 | 2026-02-15 |

**実装詳細:**
- 結果自動取得: /api/admin/scrape-results（GETでプレビュー取得）。AdminSettleFormに🤖自動取得/✏️手動入力のデュアルモードUI
- 自動清算Cron: /api/cron/auto-settle（*/10 * * * *）。post_time+10〜40分のレースを検出→スクレイプ→結果登録→settleRace実行
- 情報更新Cron: /api/cron/update-entries（*/30 0-8 * * 0,6 = JST土日9-17時）。差分検出で変更分のみDB更新
- ポイント改定: 1着的中30-300P（旧50-500P）、複勝20P（旧30P）、危険馬10-50P（旧固定10P）、完全的中200P（旧300P）。旧G1ボーナス（100P固定・1着のみ）→各的中にグレード加算する形に統一
- ランク調整: 全21段階。ビギナーⅠ(0P)→レジェンド(45,000P)。月3,500P上級者で約12ヶ月到達
- BackLink: 共通コンポーネント化。マイページ系→マイページ、ユーザー系→検索/プロフィール、その他→トップ
...
| 54 | ~~管理ダッシュボード強化~~ | ~~KPIダッシュボード~~ → Phase I-polish #80 で実装済 | — | — |
...
| 日付 | 内容 |
|------|------|
| 2026-02-15 | Phase K 完了（自動清算Cron、情報自動更新Cron、ポイント・ランク改定、ポイントガイド更新、戻るナビゲーション）。Phase I-polish 完了（TL投票表示、通知整理、大会ティザー、KPIダッシュボード）。合計69/86件完了 |
| 2026-02-15 | Phase I 完了（コメント通報/編集/削除、ユーザーブロック、管理者コメント管理、ユーザー検索、レースカレンダー、ヘルスチェックAPI、DBバックアップ戦略、ステージング環境）。58/76件完了 |
...
echo "✅ TASKLIST.md 更新完了（69/86件完了）"
