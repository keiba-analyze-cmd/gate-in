...
mkdir -p src/app/api/cron/auto-settle
cat > src/app/api/cron/auto-settle/route.ts << 'EOF'
...
EOF
echo "  ✅ src/app/api/cron/auto-settle/route.ts"

# ============================================================
# 2. vercel.json にCron追加
# ============================================================
...
EOF
echo "  ✅ vercel.json (10分おきCron)"

echo ""
echo "=================================================="
echo "🏁 自動清算Cron セットアップ完了!"
echo "=================================================="
echo ""
echo "📋 次のステップ:"
echo "  1. npm run build"
echo "  2. ビルド成功後:"
echo "     git add -A && git commit -m 'feat: レース結果自動清算Cron（10分おき）' && git push"
echo ""
echo "📝 仕組み:"
echo "  - 10分おきにCronが実行"
echo "  - post_time + 10分 経過 & 未清算 & external_id有りのレースを検出"
echo "  - netkeibaから結果を自動取得→結果登録→ポイント計算"
echo "  - 結果未公開の場合はスキップ（最大3回=30分間リトライ）"
echo "  - 40分経過しても取得できなければ対象外（管理画面から手動対応）"
echo ""
echo "⚠️ 手動設定:"
echo "  Vercel Dashboard → Settings → Environment Variables に以下を追加:"
echo "  CRON_SECRET=<任意のシークレット文字列>"
echo "  （Vercel Pro プランが必要です）"
