    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-gray-900 via-purple-900 to-gray-900 p-4">
        <div className="max-w-md mx-auto pt-12">
          <Link href="/" className="text-sm text-gray-400 hover:text-green-400 mb-4 inline-block">← トップ</Link>
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 via-purple-900 to-gray-900 text-white">
      <div className="max-w-lg mx-auto p-4 pb-24">
        ...
        <div className="text-center mb-6">
          <div className="inline-flex items-center gap-2 bg-yellow-500/20 border border-yellow-500/50 rounded-full px-4 py-1 mb-3">
            {isActive ? (
              <>
                <span className="animate-pulse text-red-500">●</span>
                <span className="text-yellow-300 text-sm font-bold">LIVE 開催中</span>
              </>
            ) : (
              <>
                <span className="text-gray-400">📊</span>
                <span className="text-gray-300 text-sm font-bold">結果発表</span>
              </>
            )}
          </div>
          <h1 className="text-2xl font-black mb-1">{contest.name}</h1>
          <p className="text-gray-400 text-sm">WIN5対象 5レースで競おう！</p>
        </div>
        ...
        <div className={`rounded-2xl p-5 mb-6 border-2 ${
          isEligible 
            ...
            : "bg-gradient-to-r from-orange-900/50 to-amber-900/50 border-orange-500"
        }`}>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className={`text-lg font-black ${isEligible ? "text-green-300" : "text-orange-300"}`}>
            ...
            {isEligible && myEntry && (
              <div className="text-right">
                <div className="text-3xl font-black text-white">{myEntry.ranking}位</div>
                <div className="text-xs text-gray-400">/ {totalParticipants}人中</div>
              </div>
            )}
          </div>
          ...
          <div className="mb-3">
            ...
            <div className="h-3 bg-gray-700 rounded-full overflow-hidden relative">
              ...
              />
              {/* 3レースの基準線 */}
              <div className="absolute left-[60%] top-0 w-0.5 h-full bg-white/50" />
            </div>
            <div className="text-right mt-1">
              <span className="text-[10px] text-gray-500">↑ 参加ライン(3レース)</span>
            </div>
          </div>
          ...
          {isEligible && myEntry && (
            <div className="flex items-center justify-center gap-6 mt-4 pt-4 border-t border-white/10">
              <div className="text-center">
                <div className="text-3xl font-black text-yellow-300">{myPoints}</div>
                <div className="text-xs text-gray-400">獲得ポイント</div>
              </div>
              <div className="w-px h-10 bg-white/20" />
              <div className="text-center">
                <div className="text-lg font-bold text-gray-300">🥇まで</div>
                <div className="text-xl font-black text-white">{pointsToFirst}pt</div>
              </div>
            </div>
          )}
        </div>
        ...
        {activeTab === "overview" && (
          <div className="space-y-4">
            ...
            <div className="bg-gray-800/50 rounded-2xl p-4 border border-gray-700">
              <h3 className="font-bold text-gray-300 mb-3">🎁 賞金</h3>
              <div className="flex justify-around">
                {[
                  ...
                  <div key={prize.rank} className="text-center">
                    ...
                    <div className="text-lg font-black text-yellow-300">¥{prize.amount.toLocaleString()}</div>
                    <div className="text-xs text-gray-500">Amazonギフト</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-gray-800/50 rounded-2xl p-4 border border-gray-700">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-bold text-gray-300">🔥 リアルタイム順位</h3>
                <span className="text-xs text-gray-500">{totalParticipants}人参加中</span>
              </div>
              <div className="space-y-2">
                {entries.slice(0, 3).map((user) => (
                  <div key={user.user_id} className={`flex items-center gap-3 p-2 rounded-lg ${
                    user.ranking === 1 ? "bg-yellow-500/10" : "bg-gray-700/30"
                  }`}>
                    <UserAvatar avatarEmoji={user.profiles.avatar_emoji} size="sm" />
                    <span className="flex-1 font-medium truncate">{user.profiles.display_name}</span>
                    <span className="font-bold text-yellow-300">{user.total_points}pt</span>
                  </div>
                ))}
                {/* 自分の順位 */}
                {myEntry && isEligible && (
                  <div className="border-t border-gray-700 pt-2 mt-2">
                    <div className="flex items-center gap-3 p-2 rounded-lg bg-purple-500/20 border border-purple-500/50">
                      <span className="text-lg">👤</span>
                      <span className="flex-1 font-medium">あなた</span>
                      <span className="text-sm text-gray-400 mr-2">{myEntry.ranking}位</span>
                      <span className="font-bold text-purple-300">{myPoints}pt</span>
                    </div>
                  </div>
                )}
              </div>
            ...
            {/* ルール */}
            <div className="bg-gray-800/50 rounded-2xl p-4 border border-gray-700">
              <h3 className="font-bold text-gray-300 mb-3">📋 ルール</h3>
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      {status === "finished" && (
                        <span className="bg-gray-600 text-white text-xs font-bold px-2 py-0.5 rounded">終了</span>
                      )}
                      {status === "open" && (
                        <span className="bg-green-600 text-white text-xs font-bold px-2 py-0.5 rounded">受付中</span>
                      )}
                    ...
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm ${
                      hasVoted ? "bg-green-500" : "bg-gray-600"
                    }`}>
                      {hasVoted ? "✓" : "−"}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-gray-400">{race.grade || ""}</span>
                    {hasVoted && status === "finished" && (
                      <span className={`text-lg font-bold ${isHit ? "text-yellow-300" : "text-gray-500"}`}>
                    ...
                    {!hasVoted && status !== "open" && (
                      <span className="text-sm text-gray-500">未予想</span>
                    )}
                  </div>
                </div>
              );
          ...
          <div className="space-y-2">
            {entries.length === 0 ? (
              <div className="rounded-xl border border-gray-700 bg-gray-800/50 p-6 text-center text-gray-400">
                ...
                <p>まだエントリーがありません</p>
                <p className="text-sm mt-1"><span className="font-bold text-green-400">3レース以上</span>予想すると自動でランキングに参加！</p>
              </div>
            ) : (
              <>
                {entries.map((user) => {
                  ...
                  return (
                    <Link
                      ...
                      className={`flex items-center gap-3 p-3 rounded-xl transition-colors ${
                        user.ranking <= 3 
                          ? "bg-gradient-to-r from-yellow-900/30 to-amber-900/20 border border-yellow-600/30" 
                          : isMe
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="font-medium truncate">
                          ...
                          {isMe && <span className="text-purple-400 text-xs ml-1">（あなた）</span>}
                        </div>
                ...
                {myEntry && isEligible && myEntry.ranking > 5 && (
                  <div className="border-t border-gray-700 pt-4 mt-4">
                    ...
                    <div className="flex items-center gap-3 p-3 rounded-xl bg-purple-900/30 border border-purple-500/50">
                      <div className="w-8 h-8 rounded-full flex items-center justify-center font-bold bg-purple-600">
                        {myEntry.ranking}
                      </div>
                    ...
                    <div className="text-center text-sm text-gray-500 mt-3">
                      🥉 3位まで あと <span className="text-yellow-300 font-bold">{pointsToThird}pt</span>
                    </div>
                  </div>
                )}
              </>
        ...
        {/* フローティングCTA（未参加時） */}
        {!isEligible && isActive && (
          <div className="fixed bottom-0 left-0 right-0 bg-gradient-to-t from-gray-900 via-gray-900 to-transparent p-4 pt-8">
            <Link 
