...
export async function GET(request: Request) {
  ...
  if (filter === "all" || filter === "hit") {
    let hitQ = admin.from("votes")
      .select("id, user_id, race_id, status, earned_points, is_perfect, like_count, copy_count, comment, settled_at, created_at, profiles(display_name, avatar_url, rank_id, is_verified), races(name, grade, course_name, race_number, race_date), vote_picks(pick_type, race_entries(post_number, horses(name)))")
  ...
  if (filter === "all" || filter === "vote") {
    let pendingQ = admin.from("votes")
      .select("id, user_id, race_id, status, like_count, copy_count, comment, created_at, profiles(display_name, avatar_url, rank_id, is_verified), races(name, grade, course_name, race_number, race_date), vote_picks(pick_type, race_entries(post_number, horses(name)))")
